...
export function POSScreen() {
  ...
  const [showUnifiedCustomerSelector, setShowUnifiedCustomerSelector] = useState(false)
  ...
  const loyaltyPointsEarned = useMemo(() => {
    ...
    // Use loyalty program point value if available, otherwise default to $1 = 1 point
    const pointValue = loyaltyProgram?.point_value || 1.00
    const earned = Math.floor(total / pointValue)

    console.log('🎁 Loyalty Points Calculation:', {
      total,
      pointValue,
      earned,
      customerPoints: selectedCustomer.loyalty_points,
  ...
  }), [products, searchQuery, selectedCategory, selectedStrainTypes, selectedConsistencies, selectedFlavors])
  ...
  ].filter(Boolean).length, [selectedCategory, selectedStrainTypes.length, selectedConsistencies.length, selectedFlavors.length])
  ...
  const matchingFiltersMap = useMemo(() => {
    ...
    filteredProducts.forEach((product) => {
      ...
      if (matchingFilters.length > 0) {
        map.set(product.id, matchingFilters)
      }
    })
    return map
  }, [filteredProducts, selectedCategory, selectedStrainTypes, selectedConsistencies, selectedFlavors])
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      {/* Unified Customer Selector - Scan ID + Search in ONE interface */}
      {vendor && (
        <POSUnifiedCustomerSelector
          visible={showUnifiedCustomerSelector}
          vendorId={vendor.id}
          onCustomerSelected={(customer) => {
            setSelectedCustomer(customer)
            setShowUnifiedCustomerSelector(false)
          }}
          onNoMatchFoundWithData={(_data: AAMVAData) => {
            // TODO: Open new customer form with pre-filled data
            setShowUnifiedCustomerSelector(false)
          }}
          onClose={() => setShowUnifiedCustomerSelector(false)}
        />
      )}
      ...
      <Animated.View style={[styles.mainLayout, {
                ...
                >
                      ...
                      {categories.map((cat, index) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            index === categories.length - 1 && styles.filterOptionLast,
                  ...
                  {availableStrainTypes.length > 0 && (
                    <View style={styles.filtersSection}>
                      ...
                      <View style={styles.filterGroupContainer}>
                        {availableStrainTypes.map((strain, index) => {
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.filterOption,
                                index === 0 && styles.filterOptionFirst,
                                index === availableStrainTypes.length - 1 && styles.filterOptionLast,
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.filterOption,
                                index === 0 && styles.filterOptionFirst,
                                index === availableConsistencies.length - 1 && styles.filterOptionLast,
                                isSelected && styles.filterOptionActive
                              ]}
                            >
                              <Text style={[
                          ...
                          return (
                            <TouchableOpacity
                              ...
                              style={[
                                styles.filterOption,
                                index === 0 && styles.filterOptionFirst,
                                index === availableFlavors.length - 1 && styles.filterOptionLast,
                                isSelected && styles.filterOptionActive
                              ]}
                            >
                              <Text style={[
        ...
        <View style={styles.rightColumn}>
          <POSCart
            ...
            onSelectCustomer={() => setShowUnifiedCustomerSelector(true)}
...
const styles = StyleSheet.create({  container: {
  ...
  // iOS 26 Grouped List Container - Rounded section
  filterGroupContainer: {
    borderRadius: 16,
    overflow: 'hidden',
  },
  // iOS 26 List Items - Match customer selector
  filterOption: {
    height: 60,
    paddingHorizontal: 20,
