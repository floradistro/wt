-- Correct create_pos_sale RPC function matching actual schema
-- Date: 2025-11-16
-- Based on 001_pos_schema.sql actual table structure

DROP FUNCTION IF EXISTS create_pos_sale CASCADE;

CREATE OR REPLACE FUNCTION create_pos_sale(
  p_location_id UUID,
  p_vendor_id UUID,  -- Not used in orders table, but keeping for compatibility
  p_session_id UUID,
  p_user_id UUID,
  p_items JSONB,
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- Get first active register for this location (or use a default)
  ...
  -- If no register found, use a dummy UUID (you should handle this better in production)
  IF v_register_id IS NULL THEN
    v_register_id := p_location_id; -- Fallback
  END IF;
  ...
  -- Create order (matching actual schema from 001_pos_schema.sql)
  INSERT INTO orders (
    order_number,
    location_id,
    register_id,
  ...
  -- Create order items (matching actual schema)
  ...
  -- Create transaction (using 'transactions' table, not 'payment_transactions')
  INSERT INTO transactions (
    ...
    'manual',  -- Or pass this as parameter
    'approved'
  ) RETURNING id INTO v_transaction_id;

  -- Update session totals (using cash_sessions table)
  ...
  IF p_customer_id IS NOT NULL THEN
    ...
    DECLARE
      ...
      v_balance_after INT;
    BEGIN
      SELECT loyalty_points INTO v_balance_before FROM customers WHERE id = p_customer_id;
      ...
      IF v_loyalty_points_earned > 0 THEN
        UPDATE customers
        SET loyalty_points = loyalty_points + v_loyalty_points_earned,
...
COMMENT ON FUNCTION create_pos_sale IS 'Creates a POS sale with order, items, payment, inventory deduction, and loyalty points - matches actual schema';
