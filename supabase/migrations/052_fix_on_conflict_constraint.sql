-- ============================================================================
-- FIX: Use constraint name in ON CONFLICT instead of column list
-- ============================================================================
-- Issue: ON CONFLICT (order_id, inventory_id) can be ambiguous
-- Fix: Use ON CONFLICT ON CONSTRAINT to explicitly reference the constraint
-- ============================================================================
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT ON CONSTRAINT inventory_holds_order_inventory_unique
    DO UPDATE SET
    ...
    -- Return hold details (using explicit column list to avoid ambiguity)
    RETURN QUERY SELECT
      v_hold_id AS hold_id,
      v_inventory_id AS inventory_id,
      v_product_id AS product_id,
      v_requested_qty AS quantity,
...
'Reserves inventory for an order before payment. Uses ON CONSTRAINT and explicit column aliases to eliminate ambiguity.';
