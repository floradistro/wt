 ...
 * - PDF-417: Driver's licenses / ID cards (AAMVA format)
 * - QR: Wallet passes (customer loyalty, order pickup) - UUID format
 */
...
export function useCameraScanner() {
  ...
  // ========================================
  // ID CARD SCANNING (PDF-417 / AAMVA)
  // ========================================
  const handleBarcodeScan = async (barcodeData: string) => {
    try {
      ...
      // Show parsed data and save to customer store
      setParsedData(data)
  ...
  // ========================================
  // MAIN SCAN HANDLER - Simple routing (PDF-417 for IDs, QR for wallet passes)
  // ========================================
  const handleCodeScanned = useCallback(
    (codes: any[]) => {
      ...
      logger.info('[CameraScanner] Scanned:', {
        type: code.type,
      ...
      if (UUID_REGEX.test(code.value)) {
        ...
        logger.info('[CameraScanner] ID barcode detected - parsing AAMVA')
  ...
  const codeScanner = useCodeScanner({
    // Only scan PDF-417 (ID cards) and QR (wallet passes)
    // No Code-128 - shipping labels not needed, orders are matched by customer
    codeTypes: ['pdf-417', 'qr'],
